...
<head>
    ...
    <title>Pelindo Multi Terminal - Anggaran Management</title>
...
<main class="container mx-auto px-3 md:px-6 py-6 flex-1">
  ...
  <div class="bg-white rounded-lg shadow-lg p-3 md:p-4 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0">
    <div class="flex flex-wrap items-center gap-3">
      <button id="addAnggaranBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-3 md:px-4 py-2 rounded-lg flex items-center space-x-2 text-sm md:text-base">
        <i class="fas fa-plus"></i> <span>Tambah Anggaran</span>
      </button>
      <a href="{{ url('/api/anggaran/export') }}" class="bg-green-600 hover:bg-green-700 text-white px-3 md:px-4 py-2 rounded-lg flex items-center space-x-2 text-sm md:text-base">
        <span>Export Excel</span> <i class="fas fa-file-excel"></i>
      </a>
    </div>
    <input type="text" id="searchInput" placeholder="Cari anggaran..." class="border rounded-lg px-3 py-2 w-full md:w-64 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
  </div>
    ...
    <table class="w-full min-w-[600px]">
      <thead class="bg-blue-600 text-white text-sm md:text-base">
        <tr>
          ...
          <th class="px-4 md:px-6 py-3 text-left font-medium">Nama Anggaran</th>
      ...
      <tbody id="anggaranTableBody" class="divide-y divide-gray-200 text-sm md:text-base"></tbody>
    </table>
  </div>
  ...
  <div id="emptyState" class="text-center py-8 hidden">
    ...
    <p class="text-gray-600">Tidak ada data anggaran</p>
  </div>
</main>
...
<!-- Modal Add/Edit -->
<div id="anggaranModal" class="modal fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 px-2">
  <div class="bg-white rounded-lg p-5 md:p-8 w-full max-w-md md:max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
    <div class="flex items-center justify-between mb-4">
      <h3 id="modalTitle" class="text-lg md:text-xl font-semibold">Tambah Anggaran</h3>
      <button id="closeModal" class="text-gray-400 hover:text-gray-600">
        <i class="fas fa-times text-xl"></i>
      </button>
    </div>

    <form id="anggaranForm" class="bg-white rounded-xl space-y-5">
      <input type="hidden" id="anggaranId">
      <input type="hidden" id="updateBy">

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">Nama Anggaran</label>
        <input type="text" id="namaAnggaran" required class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
        <div id="namaError" class="text-red-600 text-sm mt-1 hidden">
          Nama anggaran hanya boleh berisi huruf, angka dan spasi.
        </div>
      </div>
    ...
    <div class="border-t mt-6 pt-4">
      ...
      <a href="{{ url('/api/anggaran/export-template') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center justify-center space-x-2 mb-3">
...
<script>
const apiUrl = "/api/m_anggaran/paginated";
const apiUrlCRUD = "/api/m_anggaran";
const tableBody = document.getElementById("anggaranTableBody");
const loadingState = document.getElementById("loadingState");
const emptyState = document.getElementById("emptyState");
const modal = document.getElementById("anggaranModal");
const addBtn = document.getElementById("addAnggaranBtn");
...
// Validasi elements
const namaInput = document.getElementById("namaAnggaran");
...
// ==== Fetch Anggaran with Pagination ====
async function loadAnggaran(keyword = "", page = 1) {
    ...
    try {
        ...
        data.forEach((anggaran, i) => {
            const rowNumber = ((response.current_page - 1) * perPage) + i + 1;

            let row = `
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4">${rowNumber}</td>
                    <td class="px-6 py-4">${anggaran.NAMA_ANGGARAN}</td>
                    <td class="px-6 py-4">${anggaran.CREATE_BY ?? '-'}</td>
                    <td class="px-6 py-4 text-center space-x-2">
                        <button onclick="editAnggaran(${anggaran.ID_ANGGARAN})" class="text-blue-600 hover:text-blue-800"><i class="fas fa-edit"></i></button>
                        <button onclick="deleteAnggaran(${anggaran.ID_ANGGARAN})" class="text-red-600 hover:text-red-800"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
...
function renderPaginationControls(paginationData) {
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pageBtn.addEventListener("click", () => loadAnggaran(lastSearchKeyword, i));
...
prevPageBtn.addEventListener("click", () => {
    if (currentPage > 1) loadAnggaran(lastSearchKeyword, currentPage - 1);
});

nextPageBtn.addEventListener("click", () => {
    if (currentPage < totalPages) loadAnggaran(lastSearchKeyword, currentPage + 1);
});

perPageSelect.addEventListener("change", (e) => {
    perPage = parseInt(e.target.value);
    loadAnggaran(lastSearchKeyword, 1);
});
...
addBtn.addEventListener("click", () => {
    ...
    document.getElementById("modalTitle").innerText = "Tambah Anggaran";
    form.reset();
...
form.addEventListener("submit", async function(e) {
    ...
    if (!regex.test(namaValue)) {
        ...
        showToast("Nama anggaran hanya boleh huruf, angka dan spasi");
        return;
    }

    const id = document.getElementById("anggaranId").value;
    const payload = {
        NAMA_ANGGARAN: namaValue,
    ...
    try {
        ...
        if (res.ok) {
            ...
            loadAnggaran(lastSearchKeyword, currentPage);
        } else {
...
async function editAnggaran(id) {
    try {
        ...
        modal.classList.add("show");
        document.getElementById("modalTitle").innerText = "Edit Anggaran";
        document.getElementById("anggaranId").value = data.ID_ANGGARAN;
        document.getElementById("namaAnggaran").value = data.NAMA_ANGGARAN;
...
document.getElementById("importForm").addEventListener("submit", async function(e) {
    ...
    try {
        const res = await fetch("/api/anggaran/import", {
            method: "POST",
            headers: { 'Authorization': `Bearer ${token}` },
            body: formData
        });
...
document.getElementById("searchInput").addEventListener("input", function() {
    ...
    searchTimeout = setTimeout(() => loadAnggaran(keyword, 1), 500);
});
