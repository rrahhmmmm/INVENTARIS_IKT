...
<head>
    ...
    <title>Pelindo Multi Terminal - Kondisi Management</title>
...
<main class="container mx-auto px-3 md:px-6 py-6 flex-1">
    ...
    <div class="bg-white rounded-lg shadow-lg p-3 md:p-4 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0">
        <div class="flex flex-wrap items-center gap-3">
            <button id="addKondisiBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-3 md:px-4 py-2 rounded-lg flex items-center space-x-2 text-sm md:text-base">
                <i class="fas fa-plus"></i> <span>Tambah Kondisi</span>
            </button>
        </div>
        ...
        <table class="w-full min-w-[700px]">
            <thead class="bg-blue-600 text-white text-sm md:text-base">
                <tr>
                    ...
                    <th class="px-4 md:px-6 py-3 text-left font-medium">Nama Kondisi</th>
            ...
            <tbody id="kondisiTableBody" class="divide-y divide-gray-200 text-sm md:text-base"></tbody>
        </table>
    </div>
    ...
    <div id="emptyState" class="text-center py-8 hidden">
        ...
        <p class="text-gray-600">Tidak ada data kondisi</p>
    </div>
</main>
...
<!-- Modal -->
<div id="kondisiModal" class="modal fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 px-2">
    <div class="bg-white rounded-lg p-5 md:p-8 w-full max-w-md mx-auto max-h-[90vh] overflow-y-auto">
        
        <!-- Header -->
        <div class="flex items-center justify-between mb-4">
            <h3 id="modalTitle" class="text-lg md:text-xl font-semibold">Tambah Kondisi</h3>
        ...
        <!-- Form -->
        <form id="kondisiForm" class="bg-white rounded-xl space-y-5">
            <input type="hidden" id="kondisiId">

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nama Kondisi</label>
                <input type="text" id="namaKondisi" required 
...
<script>
const apiUrl = "/api/m_kondisi"; 
const tableBody = document.getElementById("kondisiTableBody");
...
async function loadKondisi(keyword = "", page = 1) {
        ...
        data.forEach((item, i) => {
            ...
            let row = `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">${item.NAMA_KONDISI}</td>
                    <td class="px-6 py-4">${item.CREATE_BY ?? '-'}</td>
                    <td class="px-6 py-4 text-center space-x-2">
                        <button onclick="editKondisi(${item.ID_KONDISI})" class="text-blue-600 hover:text-blue-800"><i class="fas fa-edit"></i></button>
                        <button onclick="deleteKondisi(${item.ID_KONDISI})" class="text-red-600 hover:text-red-800"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
...
function renderPaginationControls(paginationData) {
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pageBtn.addEventListener("click", () => loadKondisi(lastSearchKeyword, i));
...
prevPageBtn.addEventListener("click", () => {
    if (currentPage > 1) loadKondisi(lastSearchKeyword, currentPage - 1);
});

nextPageBtn.addEventListener("click", () => {
    if (currentPage < totalPages) loadKondisi(lastSearchKeyword, currentPage + 1);
});

perPageSelect.addEventListener("change", (e) => {
    perPage = parseInt(e.target.value);
    loadKondisi(lastSearchKeyword, 1);
});
...
async function loadUsername() {
    try {
        ...
        document.getElementById('createBy').value = currentUsername;
        document.getElementById('updateBy').value = currentUsername;
    } catch(err) {
        console.error(err);
    }
}
...
addBtn.addEventListener("click", () => {
    ...
    document.getElementById("modalTitle").innerText = "Tambah Kondisi";
    form.reset();
    formErrors.classList.add("hidden");
    document.getElementById("kondisiId").value = "";
    loadUsername();
});
...
form.addEventListener("submit", async function(e) {
    ...
    const id = document.getElementById("kondisiId").value;
    const payload = {
        NAMA_KONDISI: document.getElementById("namaKondisi").value,
    ...
    try {
        ...
        if (res.ok) {
            ...
            loadKondisi(lastSearchKeyword, currentPage);
...
async function editKondisi(id) {
    try {
        ...
        // Load current username for UPDATE_BY field
        await loadUsername();

        modal.classList.add("show");
        document.getElementById("modalTitle").innerText = "Edit Kondisi";

        document.getElementById("kondisiId").value = data.ID_KONDISI;
        document.getElementById("namaKondisi").value = data.NAMA_KONDISI;
        document.getElementById("createBy").value = data.CREATE_BY ?? "";
        // UPDATE_BY is already set by loadUsername()
    } catch (err) {
        console.error(err);
...
document.getElementById("searchInput").addEventListener("input", function() {
    ...
    searchTimeout = setTimeout(() => loadKondisi(keyword, 1), 500);
});
