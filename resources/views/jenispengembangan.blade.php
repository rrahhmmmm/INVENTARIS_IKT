...
<head>
    ...
    <title>Pelindo Multi Terminal - Jenis Pengembangan Management</title>
...
<main class="container mx-auto px-3 md:px-6 py-6 flex-1">
    ...
    <div class="bg-white rounded-lg shadow-lg p-3 md:p-4 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0">
        <div class="flex flex-wrap items-center gap-3">
            <button id="addJenisPengembanganBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-3 md:px-4 py-2 rounded-lg flex items-center space-x-2 text-sm md:text-base">
                <i class="fas fa-plus"></i> <span>Tambah Jenis Pengembangan</span>
            </button>
        </div>
        ...
        <table class="w-full min-w-[700px]">
            <thead class="bg-blue-600 text-white text-sm md:text-base">
                <tr>
                    ...
                    <th class="px-4 md:px-6 py-3 text-left font-medium">Nama Jenis Pengembangan</th>
            ...
            <tbody id="jenisPengembanganTableBody" class="divide-y divide-gray-200 text-sm md:text-base"></tbody>
        </table>
    </div>
    ...
    <div id="emptyState" class="text-center py-8 hidden">
        ...
        <p class="text-gray-600">Tidak ada data jenis pengembangan</p>
    </div>
</main>
...
<!-- Modal -->
<div id="jenisPengembanganModal" class="modal fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 px-2">
    <div class="bg-white rounded-lg p-5 md:p-8 w-full max-w-md mx-auto max-h-[90vh] overflow-y-auto">
        
        <!-- Header -->
        <div class="flex items-center justify-between mb-4">
            <h3 id="modalTitle" class="text-lg md:text-xl font-semibold">Tambah Jenis Pengembangan</h3>
        ...
        <!-- Form -->
        <form id="jenisPengembanganForm" class="bg-white rounded-xl space-y-5">
            <input type="hidden" id="jenisPengembanganId">

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nama Jenis Pengembangan</label>
                <input type="text" id="namaPengembangan" required 
...
<script>
const apiUrl = "/api/m_tingkatpengembangan"; 
const tableBody = document.getElementById("jenisPengembanganTableBody");
const loadingState = document.getElementById("loadingState");
const emptyState = document.getElementById("emptyState");
const modal = document.getElementById("jenisPengembanganModal");
const addBtn = document.getElementById("addJenisPengembanganBtn");
const closeModal = document.getElementById("closeModal");
const cancelBtn = document.getElementById("cancelBtn");
const form = document.getElementById("jenisPengembanganForm");
...
async function loadJenisPengembangan(keyword = "", page = 1) {
        ...
        data.forEach((item, i) => {
            ...
            let row = `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">${item.NAMA_PENGEMBANGAN}</td>
                    <td class="px-6 py-4">${item.CREATE_BY ?? '-'}</td>
                    <td class="px-6 py-4 text-center space-x-2">
                        <button onclick="editJenisPengembangan(${item.ID_TINGKATPENGEMBANGAN})" class="text-blue-600 hover:text-blue-800"><i class="fas fa-edit"></i></button>
                        <button onclick="deleteJenisPengembangan(${item.ID_TINGKATPENGEMBANGAN})" class="text-red-600 hover:text-red-800"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
...
function renderPaginationControls(paginationData) {
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pageBtn.addEventListener("click", () => loadJenisPengembangan(lastSearchKeyword, i));
...
prevPageBtn.addEventListener("click", () => {
    if (currentPage > 1) loadJenisPengembangan(lastSearchKeyword, currentPage - 1);
});

nextPageBtn.addEventListener("click", () => {
    if (currentPage < totalPages) loadJenisPengembangan(lastSearchKeyword, currentPage + 1);
});

perPageSelect.addEventListener("change", (e) => {
    perPage = parseInt(e.target.value);
    loadJenisPengembangan(lastSearchKeyword, 1);
});
...
addBtn.addEventListener("click", () => {
    ...
    document.getElementById("modalTitle").innerText = "Tambah Jenis Pengembangan";
    form.reset();
    formErrors.classList.add("hidden");
    document.getElementById("jenisPengembanganId").value = "";
    loadUsername();
});
...
form.addEventListener("submit", async function(e) {
    ...
    const id = document.getElementById("jenisPengembanganId").value;
    const payload = {
        NAMA_PENGEMBANGAN: document.getElementById("namaPengembangan").value,
    ...
    try {
        ...
        if (res.ok) {
            ...
            loadJenisPengembangan(lastSearchKeyword, currentPage);
...
// ==== Edit ====
async function editJenisPengembangan(id) {
    try {
        ...
        modal.classList.add("show");
        document.getElementById("modalTitle").innerText = "Edit Jenis Pengembangan";

        document.getElementById("jenisPengembanganId").value = data.ID_TINGKATPENGEMBANGAN;
        document.getElementById("namaPengembangan").value = data.NAMA_PENGEMBANGAN;
...
// ==== Delete ====
async function deleteJenisPengembangan(id) {
...
document.getElementById("searchInput").addEventListener("input", function() {
    ...
    searchTimeout = setTimeout(() => loadJenisPengembangan(keyword, 1), 500);
});
